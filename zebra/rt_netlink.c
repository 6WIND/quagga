  ...
  if (rtm->rtm_family == AF_INET)
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        {
          if (h->nlmsg_type == RTM_NEWROUTE)
            zlog_debug ("RTM_NEWROUTE %s/%d vrf %u",
                       inet_ntoa (p.prefix), p.prefixlen, vrf_id);
          else
            zlog_debug ("RTM_DELROUTE %s/%d vrf %u",
                       inet_ntoa (p.prefix), p.prefixlen, vrf_id);
        }
...
netlink_route (int cmd, int family, void *dest, int length, void *gate,
               int index, int zebra_flags, vrf_id_t vrf_id)
{
  int ret;
  int bytelen;
  struct sockaddr_nl snl;
  ...
  req.r.rtm_table = zebrad.rtm_table_default;
  ...
  ret = netlink_talk (&req.n, &zvrf->netlink_cmd, zvrf);
  if (ret < 0)
    return -1;
