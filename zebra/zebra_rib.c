		...
		union prefix46constptr pp, const struct rib * rib)
{
  ...
  zlog_debug ("%s: dumping RIB entry %p for %s/%d vrf %u", func, rib, straddr,
              p->prefixlen, rib->vrf_id);
  zlog_debug
  (
		 ...
		 struct in_addr *gate, unsigned int ifindex, vrf_id_t vrf_id, safi_t safi)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    {
      if (gate)
	zlog_debug ("rib_delete_ipv4(): route delete %s/%d vrf %u via %s ifindex %d",
  ...
  if (! rn)
	...
	{
	  if (gate)
	    zlog_debug ("route %s/%d vrf %u via %s ifindex %d doesn't exist in rib",
		       inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN),
		       p->prefixlen, vrf_id,
  ...
  if (! same)
      ...
      else
	{
	  if (IS_ZEBRA_DEBUG_KERNEL)
		...
		zlog_debug ("route %s/%d vrf %u ifindex %d type %d doesn't exist in rib",
			   inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN),
			   p->prefixlen, vrf_id,
			   ifindex,
			   type);
		...
		struct in6_addr *gate, unsigned int ifindex, vrf_id_t vrf_id)
{
  ...
  if (type == ZEBRA_ROUTE_KERNEL && IN6_IS_ADDR_UNSPECIFIED (&p->prefix)
    ...
    {
      kernel_delete_ipv6_old (p, gate, ifindex, 0, vrf_id);
      return 1;
    }
  return 0;
}
	      ...
	      u_int32_t metric, u_char distance, safi_t safi)
{
  ...
  /* Filter bogus route. */
  if (rib_bogus_ipv6 (type, p, gate, ifindex, vrf_id))
    return 0;
