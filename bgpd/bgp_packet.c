...
*/
static struct stream *
bgp_withdraw_packet (struct peer *peer, afi_t afi, safi_t safi, int low)
{
  struct stream *s;
  struct stream *packet;
  struct bgp_adj_out *adj;
  ...
  while ((adv = (low ? BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->withdraw_low) :
                       BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->withdraw))) != NULL)
    {
      assert (adv->rn);
      adj = adv->adj;
      rn = adv->rn;
...
void
bgp_default_update_evpn_send(struct peer *peer, struct prefix_rd *rd,
                             struct attr *attr, afi_t afi,
                             size_t nlabels, uint32_t *labels)
{
  struct bgp *bgp;
  ...
  bgp_packet_mpattr_prefix(s, AFI_L2VPN, SAFI_EVPN, &p, rd, labels, nlabels, attr);
...
void
bgp_default_withdraw_evpn_send (struct peer *peer, afi_t afi, struct prefix_rd *rd)
{
  struct stream *s;
  struct prefix p;
  unsigned long attrlen_pos = 0;
  ...
  /* Encode the prefix in MP_UNREACH_NLRI attribute */
  bgp_packet_mpunreach_prefix(s, &p, AFI_L2VPN, SAFI_EVPN, rd, labels, nlabels, NULL);
...
bgp_write_packet (struct peer *peer)
{
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
	...
	if (adv)
	  {
	    s = bgp_withdraw_packet (peer, afi, safi, 0);
	    if (s)
	      return s;
	  }

	adv = BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->withdraw_low);
	if (adv)
	  {
	    s = bgp_withdraw_packet (peer, afi, safi, 1);
	    if (s)
	      return s;
	  }
      }
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
      {
	adv = BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->update_high);
	if (adv)
	  {
		...
		if (CHECK_FLAG (adv->binfo->peer->cap, PEER_CAP_RESTART_RCV)
		    ...
		    && ! CHECK_FLAG (adv->binfo->flags, BGP_INFO_STALE))
		  {
		    if ((CHECK_FLAG (adv->binfo->peer->af_sflags[afi][safi],
		      ...
		      s = bgp_update_packet (peer, afi, safi, adv);
		  }
		else
		  s = bgp_update_packet (peer, afi, safi, adv);
	      }
...
bgp_write_proceed (struct peer *peer)
{
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
	  ...
	  FIFO_HEAD (&peer->sync[afi][safi]->withdraw_low))
	return 1;

  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
      {
	if ((adv = BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->update_high)) != NULL)
...
int bgp_eor_send_afi_safi (struct thread *t)
{
  ...
  afi = ctxt->afi;
  safi = ctxt->safi;
  XFREE (MTYPE_BGP_ENDOFRIB_CTXT, ctxt);
  if (peer->afc_nego[afi][safi] && peer->synctime &&
      CHECK_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_EOR_SEND))
    return 0;
      ...
      switch (i)
        {
          case NLRI_UPDATE:
          case NLRI_MP_UPDATE:
            if (nlris[i].afi == AFI_L2VPN &&
                nlris[i].safi == SAFI_EVPN)
