                     ...
                     struct bgp_info *new_select)
{
  ...
  if (old)
    {
      ...
      if (del_last)
        {
          bgp_vrf_update (vrf, afi, vrf_rn, old, false);
  ...
  if (sel && new)
    {
      ...
      bgp_vrf_update (vrf, afi, vrf_rn, new, true);
      return;
    }
  ...
  bgp_vrf_update (vrf, afi, vrf_rn, newsel, true);
}
    ...
    for (i = 0; i < (size_t)old_ecom->size; i++)
      {
        ...
        for (ALL_LIST_ELEMENTS_RO(rt_sub->vrfs, node, vrf))
          bgp_vrf_process_two (vrf, afi, safi, rn, withdraw ? NULL : new_select);
      }
