...
bgp_vrf_static_set (struct bgp_vrf *vrf, afi_t afi, const struct bgp_api_route *route)
{
  ...
  if ((afi != AFI_IP) && (afi != AFI_INTERNAL_L2VPN))
    return -1;
  if(afi == AFI_INTERNAL_L2VPN)
    safi = SAFI_INTERNAL_EVPN;
  else
  ...
  if ((safi == SAFI_INTERNAL_EVPN) && PREFIX_IS_L2VPN_AD(p))
    {
      struct bgp *bgp;
...

{
  ...
  /* configured entries on old entry are withdrawn */
  if (old_select
  ...
  /* a new bgp vrf static entry is automatically exported to
   * other RTs mentioned in its vrf export list
   */
  if (new_select
      && new_select->type == ZEBRA_ROUTE_BGP
      && new_select->sub_type == BGP_ROUTE_STATIC)
	    ...
	    for (ri = rm->info; ri; ri = ri->next)
	      {
                ...
                if (CHECK_FLAG (ri->flags, BGP_INFO_VPN_HIDEN))
                  {
                    bgp_vrf_process_imports2(bgp, afi, safi, rm, NULL, ri, ri->attr);
                    if (!CHECK_FLAG (ri->flags, BGP_INFO_VPN_HIDEN))
...
void bgp_vrf_added_vrf_update_global_rib (struct bgp_vrf *vrf)
{
  ...
  bgp_vrf_update_global_rib_perafisafi (vrf, AFI_INTERNAL_L2VPN, SAFI_INTERNAL_EVPN);
}
...
{
  struct eth_segment_id *info_eth_s_id;
  union gw_addr *info_gw_ip;
  char temp[16];
  ...
  if(!memcmp(info_gw_ip, gw_ip, sizeof(union gw_addr)))
    return false;
  return !memcmp(info_eth_s_id, eth_s_id, sizeof(struct eth_segment_id));
}
            ...
            int soft_reconfig, struct bgp_route_evpn* evpn)
{
  ...
  if (!evpn)
  {
  ...
  else if (evpn && ((evpn->auto_discovery_type & EVPN_ETHERNET_AD_PER_ESI)
                    ||(evpn->auto_discovery_type & EVPN_ETHERNET_AD_PER_EVI)))
    {
      ...
      bgp_evpn_process_imports2(bgp, NULL, ad);
      return 0;
    }
  ...
  if(evpn && evpn->auto_discovery_type & (EVPN_ETHERNET_AD_PER_ESI|EVPN_ETHERNET_AD_PER_EVI))
    {
      ...
      bgp_evpn_process_imports2(bgp, ad, NULL);
      return 0;
    }
  /* Lookup node. */
  rn = bgp_afi_node_get (bgp->rib[afi][safi], afi, safi, p, prd);
...
bgp_route_init (void)
{
 ...
 /* BGP dampening clear commands */
  ...
  install_element (ENABLE_NODE, &show_ip_bgp_vrf_neighbor_received_cmd);
  install_element (ENABLE_NODE, &show_ipv6_bgp_vrf_cmd);
  ...
  install_element (ENABLE_NODE, &show_ipv6_bgp_vrf_route_cmd);
