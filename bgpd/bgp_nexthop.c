...
zlookup_query (struct in_addr addr)
{
  ...
  s = zlookup->obuf;
  stream_reset (s);
  zclient_create_header (s, ZEBRA_IPV4_NEXTHOP_LOOKUP, LTID_DEFAULT);
...
zlookup_query_ipv6 (struct in6_addr *addr)
{
  ...
  s = zlookup->obuf;
  stream_reset (s);
  zclient_create_header (s, ZEBRA_IPV6_NEXTHOP_LOOKUP, LTID_DEFAULT);
                  ...
                  struct in_addr *igpnexthop)
{
  ...
  s = zlookup->obuf;
  stream_reset (s);
  zclient_create_header (s, ZEBRA_IPV4_IMPORT_LOOKUP, LTID_DEFAULT);
